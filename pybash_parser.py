...
class pybash_parser():
    ...
    def expand_designators(self, cmd):
        ...
        while True:
            ...
            # Step 2) Determine which history line to use
            #   - output: 
            ...
            # HERE: write an alternative to readline.remove_history_item()
            # and use it to edit the the history once substitutions are done
            ...
            if parse_word_designator:
                ...
                expansion_performed = True
                # Sub-in entire history line
                if history_line < 0:
                    history_line = readline.get_current_history_length() + history_line

                if next_loc < len(cmd) - 1:
                    cmd = cmd[0:loc] + readline.get_history_item(history_line) + cmd[next_loc:]
                else:
                    cmd = cmd[0:loc] + readline.get_history_item(history_line)
        
        # Edit history if required
